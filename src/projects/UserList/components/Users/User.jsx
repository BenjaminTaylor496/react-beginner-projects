...
export const User = ({ id, email, first_name, last_name, avatar, onClickInvite, isInvited }) => (
	<li>
		...
		<img
			/**При клике на img будет вызываться функция 'onClickInvite' где внутрь нее будет передаваться id */
			onClick={() => onClickInvite(id)}
			className='action'
			src={`/assets/${isInvited ? 'minus' : 'plus'}.svg`}
			/**В src делается определение того: находится ли тот или иной пользователь в массиве или нет.
			 * Если пользователь был приглашен, то знак + сменится на знак -
			 */
			alt='Action'
		/>
	</li>
